...
async function getBlockTransactionsByHeight(tick, blockhash, currentBlock) {
  if (blockhash) {
    try {
      ...
      if (res.data.result) {
        ...
        for (let txid of txArray) {
          let info = await db.selectTxInfoById(tick, txid)
        ...
        //把处理完的block写入数据库
        await db.insertIndexBlockInfo(tick, currentBlock)
      }
    } catch (err) {
      console.log(err)
      if (err instanceof MaxRetriesReachedError) {
  ...
  while (true) {
    try {
      ...
      if (res.data && res.data.result && delayHash) {
        await getBlockTransactionsByHeight(tick, res.data.result, currentBlock)
        currentBlock++
      } else {
      ...
      // 打印错误信息，并等待 10 秒
      logger.error(`block ${currentBlock} Error: ${err.message}`)
